...
-- Landscape only (D-CHOMP-072). CHOMP-SYS-032 asked for both orientations; the
...
--
-- ScreenOrientation lives on PLAYERGUI, not on a ScreenGui. Setting it on the
-- ScreenGui threw, and because that line ran BEFORE gui.Parent was assigned it
-- killed the whole script on the way in - so the entire HUD disappeared, not
-- just the new buttons. Hence both fixes: it is set on the right object, and it
-- is set AFTER the HUD is parented and inside a pcall, so no piece of optional
-- polish can ever again take the whole display down with it.
if CONTROLS.LockLandscape then
	local ok, err = pcall(function()
		playerGui.ScreenOrientation = Enum.ScreenOrientation.LandscapeSensor
	end)
	if not ok then
		warn("[ChompHud] could not lock orientation: " .. tostring(err))
	end
end
