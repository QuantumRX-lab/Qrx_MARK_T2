...
-- Ghosts are anchored puppets moved with PivotTo, so physics cannot stop them
-- at a wall. Sweep a body-sized box through the authored wall set and turn any
-- blocked movement into travel along the wall face.
local wallCast = RaycastParams.new()
wallCast.FilterType = Enum.RaycastFilterType.Include
wallCast.IgnoreWater = true
local wallOverlap = OverlapParams.new()
wallOverlap.FilterType = Enum.RaycastFilterType.Include

local function refreshWallFilter()
	local walls = CollectionService:GetTagged("Chomp_Wall")
	wallCast.FilterDescendantsInstances = walls
	wallOverlap.FilterDescendantsInstances = walls
end
...
local function slideAlongWalls(here: Vector3, step: Vector3, goal: Vector3): Vector3
	...
	if not hit then return step end

	local normal = Vector3.new(hit.Normal.X, 0, hit.Normal.Z)
	if normal.Magnitude < 0.001 then return Vector3.zero end
	normal = normal.Unit
	local slide = step - normal * step:Dot(normal)

	-- A straight-on pursuit has no tangent component. Choose the direction along
	-- the wall that makes the most progress toward the target, so the ghost
	-- searches for an opening instead of looking switched off.
	if slide.Magnitude < step.Magnitude * 0.15 then
		local tangent = Vector3.new(-normal.Z, 0, normal.X)
		local towardGoal = Vector3.new(goal.X - here.X, 0, goal.Z - here.Z)
		if tangent:Dot(towardGoal) < 0 then tangent = -tangent end
		slide = tangent * step.Magnitude
	end

	-- Corners need a second sweep; otherwise sliding along one wall can carry
	-- the puppet through the wall meeting it.
	if Workspace:Blockcast(CFrame.new(here), Vector3.new(8, 8, 8), slide, wallCast) then
		return Vector3.zero
	end
	return slide
end
...
local function clearPointNear(position: Vector3): Vector3
	local limit = L.OuterRadius - L.RingSpacing
	for ring = 0, 8 do
		local radius = ring * 7
		local samples = ring == 0 and 1 or 12
		for sample = 1, samples do
			local angle = math.pi * 2 * ((sample - 1) / samples)
			local candidate = position + Vector3.new(math.cos(angle) * radius, 0, math.sin(angle) * radius)
			local flat = Vector3.new(candidate.X, 0, candidate.Z)
			if flat.Magnitude > limit then flat = flat.Unit * limit end
			candidate = pushClear(Vector3.new(flat.X, 8, flat.Z))
			local overlaps = Workspace:GetPartBoundsInBox(CFrame.new(candidate),
				Vector3.new(10, 8, 10), wallOverlap)
			if #overlaps == 0 and not sanctuaryAt(candidate) then return candidate end
		end
	end
	return position
end
...
local function startWave(n: number)
	...
	for index = 0, wanted - 1 do
		...
		here = clearPointNear(Vector3.new(x, 8, z))
		x, z = here.X, here.Z
...
local function regroupSurvivors()
	local players = activePlayers()
	if #players == 0 then return end
	local player = players[math.random(1, #players)]
	local target = player.Character and player.Character:FindFirstChild("HumanoidRootPart") :: BasePart?
	if not target then return end
	local survivors = {}
	for _, ghost in ghosts do
		if ghost.model.Parent and ghost.model:GetAttribute("Dead") ~= true then
			table.insert(survivors, ghost)
		end
	end
	for index, ghost in survivors do
		local angle = math.pi * 2 * ((index - 1) / math.max(1, #survivors))
		local wanted = target.Position + Vector3.new(math.cos(angle) * 85, 0, math.sin(angle) * 85)
		local point = clearPointNear(Vector3.new(wanted.X, 8, wanted.Z))
		ghost.radius = Vector3.new(point.X, 0, point.Z).Magnitude
		ghost.angle = math.atan2(point.Z, point.X)
		ghost.model:PivotTo(CFrame.new(point))
	end
end
...
RunService.Heartbeat:Connect(function(dt)
	...
	for index, g in ghosts do
		...
		step = slideAlongWalls(here, step, goal)
