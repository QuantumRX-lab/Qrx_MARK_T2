  ...
  try {
    if (isFreeCodeAttempt) {
      ...
      if (!lsRes.ok || !lsData.activated) {
        // Feeds the same repeated_key_failure signal validate-key-lotm.js
        // already feeds, so brute-forcing licence keys directly against this
        // endpoint (skipping /api/validate-key-lotm) still accumulates
        // toward detection.
