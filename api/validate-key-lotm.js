// /api/validate-key
//
// Tentatively checks a Lemon Squeezy licence key for the Lord of the Memes
// Forge, WITHOUT consuming its activation. The real, consuming check happens
// in /api/generate at the moment of actual card generation — see that file
...
// looks right before they even select character/flavor/mood/palette.
//
...
// never been activated yet (confirmed empirically against the QRx Forge
// build, 2026-06-16), even though the key is genuinely valid and unused.
// We treat that specific case as a tentative pass rather than a rejection,
// since rejecting it would block every legitimate first-time buyer.
// Genuinely malformed, disabled, or expired keys are still caught
// correctly below.
//
...
export default async function handler(req, res) {
  ...
  // enforcement (KV-counted, 100 redemptions). This check is tentative
  // only, same as the LS path below: it tells the frontend the code is
  // recognized, but doesn't guarantee a slot is still free. If the cap
  // has been hit, /api/generate will still correctly reject at the
  // point of actual generation.
  if (licenceKey.trim().toUpperCase() === 'LOTMFREE') {
  ...
  // Reject obviously-fake keys early
