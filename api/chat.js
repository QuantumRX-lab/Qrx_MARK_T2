...
// v2 CHANGES: added HOT TAKE to the three-part response format and to the
// pre-written SPECIFIC STORY CONTEXT direct-use fields, matching hot_take
// now generated by news-refresh.js's editorial card pass.
...
When discussing a specific story or signal, use this four-part structure with these exact labels:
...
HOT TAKE: one sentence of genuine editorial opinion, whatever tone fits the story -- skeptical, bullish, dismissive, excited. This is QuantumRx's own voice, not a summary. Never hedge it with "some say", "it remains to be seen", or similar.

After the four parts, on its own line, write: READLINK:<category> where category is one of aimoves, energy, space, crypto, policy, hot, robotics, semis, quantum -- matching the story's vertical. This will be converted into a link by the frontend, do not write it as a sentence.
...
function buildSystemPrompt(briefing, signalsContext, pageContext, storyContext) {
  ...
  // This now includes HOT TAKE when the story object carries a pre-written hot_take field
  // (generated by news-refresh.js's editorial card pass) — use it verbatim, do not soften
  // or hedge it, and do not generate a new one if it is already present.
  if (storyContext) {
